...
const Header = () => {
  ...
  return (
    <nav className='w-full flex bg-gray-600 justify-between items-center gap-1 lg:px-16 px-6 py-4 sticky top-0 z-50'>
      ...
      <ul className='lg:flex justify-center items-center gap-6 hidden'>
        {navItems.map(({ link, path }) => (
          <Link key={path} className=' text-white uppercase font-semibold cursor-pointer p-3 rounded-full hover:bg-red-600  hover:text-white font-rubik text-[15px]' to={path} spy={true} offset={-100} smooth={true}>{link}</Link>
        ))}
      </ul>
